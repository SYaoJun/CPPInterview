...
- 通过实现Runnable接口的特点，资源类是一个单独的类，线程和资源类解耦。
- 通过实现Callable接口的特点，有返回值，会阻塞，可抛出异常，有缓存。
...
锁是面向使用者，同步器是面向锁的实现者。
AQS为每个共享资源都设置了一个共享资源锁，线程在需要访问共享资源时首先需要获取共享资源锁，如果获取到了共享资源锁，便可以在当前线程中使用该共享资源，如果获取不到，则将线程放入线程等待队列，等待下一次调度。
AQS维护了一个volatile int类型的变量state，用于表示当前的同步状态。
